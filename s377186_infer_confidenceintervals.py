...
# Calculate total screen time for 5 weekdays and 2 weekends
...
#Run descriptive analysis to define screen time groups
...
mode_value = merged_df['average_daily_screen_time'].mode()
...
print("\tMode value: %.2f hours daily" % mode_value.item())
print("\tAt 25%% IQR: %.2f hours daily" % low_threshold)
print("\tAt 75%% IQR: %.2f hours daily" % high_threshold)
print("Additionally, one recent study in 2024 shows that the average daily screen time for teens from 13-18 is at 8 hours 39 minute (https://explodingtopics.com/blog/screen-time-for-teens)")
print("From all the information above, we determine the screen time groups as follow:\n\tLow: Less than %.2f hours a day\n\tMedium: From %.2f hours to %.2f hours a day\n\tHigh: More than %.2f hours a day"%(low_threshold,low_threshold,high_threshold,high_threshold))
...
# Plot histograms for each screen time group 
